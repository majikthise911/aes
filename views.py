...
def show_ai_report(proposals):
    ...
    col1, col2 = st.columns(2)
    with col1:
        # Export executive summary (extracted from existing report - no API call)
        if st.button("📥 Executive Summary", use_container_width=True, key="recommendation_exec_summary"):
            try:
                # Extract first ~500 words from the report (top sections)
                full_report = st.session_state.generated_report
                lines = full_report.split('\n')

                # Take first 50 lines or until we hit detailed analysis sections
                summary_lines = []
                for i, line in enumerate(lines[:60]):
                    # Stop at detailed analysis sections
                    if any(stop_word in line.lower() for stop_word in ['detailed analysis', 'comprehensive breakdown', 'full comparison']):
                        break
        ...
        if analysis.get('error'):
            ...
            with col2:
                if st.button("📥 Executive Summary", use_container_width=True, key="scope_exec_summary"):
                    ...
                    # Extract top sections from final assessment (no API call)
                    try:
                        final_assessment = analysis.get('final_assessment', 'N/A')
                        rankings = analysis.get('rankings', '')
                        key_findings = analysis.get('key_findings', '')

                        # Build concise summary from existing data
                        exec_summary_content = f"{rankings}\n\n{key_findings}"
                        if final_assessment and final_assessment != 'N/A':
                            # Take first 40 lines of final assessment
