...
class GPTExtractor:
    ...
    def _extract_from_single_chunk(self, text: str) -> Dict:
...

        {
            ...
            "bid_assumptions": {
                "pricing_type": "string (Firm, Indicative, etc.)",
                "bid_date": "string",
                "pv_design_basis": "string (e.g., '30% design', 'IFC')",
                "hv_design_basis": "string",
                "work_schedule": "string (e.g., '10 hours, 6 days/week')",
                "labor_rate": "string (Open Shop, Prevailing Wage, Union)",
                "sales_tax_exemption": "string or float",
                "module_type": "string",
                "inverter_type": "string",
                "racking_type": "string",
                "pile_size": "string",
                "pile_galvanized": "string (Y/N, mil thickness)",
                "predrill_percentage": "string or float",
                "predrill_cost_per_pile": float,
                "pile_refusal_percentage": "string or float",
                "racking_spares": "string",
                "domestic_content_steel": "string (Y/N, details)",
                "domestic_content_tracker": "string (Y/N, details)"
            },
            "scope": {
        ...
        - For bid_assumptions: Look for bid assumptions table, project basis info, or proposal clarifications
        - Common bid assumption fields: pricing type (firm/indicative), design basis (30%/IFC), labor type (union/prevailing wage/open shop), module/inverter/racking specs, pile details, domestic content requirements
        - Extract pile information: size (e.g., "W6x15"), galvanization, pre-drill %, refusal %, cost per pile
        - Look for IRS domestic content assumptions for steel and tracker components
    ...
    def _merge_extraction_results(self, results: List[Dict]) -> Dict:
        ...
        for result in results[1:]:
            ...
            for section in ['technology', 'capacity', 'equipment', 'costs', 'detailed_sov', 'scope', 'schedule', 'bid_assumptions']:
...
Return the enhanced recommendation report."""
    ...
    def generate_epc_recommendation_report(self, proposals_data: List[Dict], scope_analysis: Dict = None, parallel: bool = True) -> str:
        ...
        # Get scope analysis summary if available
        scope_summary = ""
        if scope_analysis:
            scope_summary = f"""
...
"""
    ...
    def _chain1_initial_recommendation(self, proposals_summary: str, scope_summary: str) -> str:
        """Chain 1: Initial comprehensive analysis."""

        prompt = f"""You are an expert EPC consultant conducting the INITIAL ANALYSIS phase.

Analyze the proposals and provide:

1. **COST ANALYSIS**
   - Total cost comparison and ranking
   - Cost per watt analysis
   - Cost breakdown evaluation (equipment, labor, materials)
   - Value assessment

2. **TECHNICAL ANALYSIS**
   - Equipment quality comparison (modules, inverters, racking)
   - Technology approach evaluation
   - Technical specifications assessment

3. **SCOPE ANALYSIS**
   - Comprehensiveness of each proposal's scope
   - Key assumptions and their implications
   - Important exclusions and risks
   - Clarity of deliverables

4. **INITIAL CONTRACTOR ASSESSMENT**
   - Strengths of each contractor's proposal
...
Provide objective, data-driven analysis. Be thorough and identify any red flags or concerns."""
    ...
    def _chain2_deep_evaluation(self, proposals_summary: str, scope_summary: str, chain1_analysis: str) -> str:
        ...
        prompt = f"""You are an expert EPC consultant conducting DEEP EVALUATION phase.

Review the initial analysis and provide:
...
3. **VALUE ENGINEERING PERSPECTIVE**
   - Where can costs be optimized?
   - Are premium costs justified?
   - Hidden value in proposals

4. **STRATEGIC CONSIDERATIONS**
   - Contractor reputation and track record implications
   - Equipment quality and long-term reliability
...
Synthesize both analyses into a cohesive trade-off and risk assessment. Add any considerations from the initial analysis that affect the risk profile. Keep response concise."""
    ...
    def _chain3_final_recommendation(self, proposals_summary: str, scope_summary: str,
        ...
        prompt = f"""You are the lead EPC consultant providing the FINAL RECOMMENDATION.

Review both previous analyses and create a comprehensive executive report.

**Your task:**
1. Synthesize findings from both analysis phases
2. Identify any contradictions or gaps in reasoning
3. Provide clear, actionable recommendation
4. Structure as a professional executive report

**Required report structure:**

## EXECUTIVE SUMMARY
- Top recommended EPC contractor
- Key reasoning (2-3 sentences)
- Estimated cost and value proposition

## DETAILED FINDINGS

### Financial Analysis
[Cost comparison, value assessment, budget implications]

### Technical Evaluation
[Equipment quality, technology approach, specifications]

### Scope & Risk Assessment
[Scope comprehensiveness, key risks, mitigation strategies]

### Contractor Comparison
[Head-to-head comparison with strengths/weaknesses]

## FINAL RECOMMENDATION

### Primary Recommendation
[Contractor name, justification, expected outcomes]

### Risk Mitigation Plan
[Key risks and how to address them]

### Alternative Options
- If budget is primary concern: [recommendation]
- If quality is primary concern: [recommendation]
- If timeline is critical: [recommendation]

## NEXT STEPS
[Actionable items for moving forward]
